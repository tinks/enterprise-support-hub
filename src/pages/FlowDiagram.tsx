    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "Report §2b 'Triage time' (SlaReport.tsx, presentational only — same month window + inScope population as the rest of the report). Evaluable set = rows with hasSeverityEvent && timeToTriageS != null. Shows median / average / p90 / n on wall-clock (median is the 'single global number'), plus the business-hours median as a clearly-labelled secondary so off-hours arrivals don't read as slow work, plus a by-FINAL-severity mini-table (Sev 1–4 + Unclassified, median/avg/n) and a count of tickets re-classified after first triage. HONEST COVERAGE LINE is mandatory: 'Triage measurable for N of M in-scope tickets' with the note that event_details capture only starts ~15 July 2026 — tickets closed before that are NOT-EVALUABLE, not fast, and the evaluable subset is never presented as the whole population. A measure-first note states no target is set yet and that this distribution is the basis for proposing one (the what-if slider can tune it later).",
