    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "First Response (compliance): firstHumanReplyFromInboxS / firstHumanReplyFromInboxBusinessHoursS — first human_admin public reply AT/AFTER the anchor (Sam and bots excluded). Replies before the anchor are ignored. Null when no post-anchor human reply.",
          "Resolution (compliance): resolutionActiveS / resolutionActiveBusinessHoursS — stop-the-clock active in-our-court walk from the anchor to last close, excluding customer-wait segments and closed-then-reopened gaps (guard: closeAt < anchor → 0). Industry-standard ball-in-our-court + inbox-anchored so Sam's pre-handoff work is out. Raw ttr* preserved for reference only.",
          ...
          "UI /sla-test — Tab 2 'Batch (stored)': snapshot-based, NOT live. Reads intercom_tickets_v3 filtered by lifecycle_status IN ('finalized','reopened_after_finalize') — full finalized enterprise population, ALL owners, paged 500 (~287 rows). Cleans population: manuallyLogged first, then excluded (not_enterprise / enterprise-fyi / enterprise-duplicate / merged_ticket / rsa_override=false; rsa_override=true overrides), noCustomer, inScope. Compliance section rendered at TOP of the batch view; KPI cards below now aggregate the inbox-anchored fields (Human first reply · bus.hrs = firstHumanReplyFromInboxBusinessHoursS; Human first reply · calendar = firstHumanReplyFromInboxS) plus a new 'Pre-inbox time (pre-Enterprise / work-before-ticket)' card explicitly labelled as a process signal not an SLA. Snapshot-not-live because live-fetching a whole population per page load would hammer the metered Intercom API — snapshot is fast, free, reproducible, as-of-a-time.",
