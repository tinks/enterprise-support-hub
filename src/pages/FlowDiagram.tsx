    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "TRIAGE METRIC — MEASURE-FIRST, no target (computeTriage). Time from the Enterprise-Inbox anchor (slaClockStartS) to the FIRST Severity assignment. Fields on SlaResult: firstSeverityAtS, firstSeverityValue, timeToTriageS (wall-clock), timeToTriageBusinessHoursS (Berlin business clock), severityEventCount, hasSeverityEvent. NULL / not-evaluable when there is no Severity event or no anchor — deliberately never defaulted to 0; a Severity event BEFORE the anchor clamps to 0. No target and no compliance verdict yet: the distribution is the evidence from which a target gets proposed. Why it matters: triage time is SEVERITY-AGNOSTIC, which is why it (unlike a blended FRT average across severities with different clocks) legitimately supports a SINGLE GLOBAL number for leadership.",
          ...
          "TRIAGE OVERRIDES — public.triage_overrides (migration 2e4fbbd) + SlaWorkbench 'Triage violations' section (commit e9b3bae). Mirrors sla_breach_overrides in shape and styling but is TEAM-WRITABLE: any authenticated teammate can excuse a violation with a reason + note, created_by records WHO (from supabase.auth.getSession() email), and DELETE is ADMIN-ONLY so the audit trail survives — RLS enforces it, the 'clear override' control is simply hidden for non-admins. One row per intercom_conversation_id (UNIQUE), saved via upsert onConflict=intercom_conversation_id, so editing an existing override is the same call. Reasons: off_hours | non_support_thread | recorded_at_close | answered_before_classified | genuine_miss | other; the dropdown AUTO-SUGGESTS recorded_at_close, else answered_before_classified, from the engine flags. The section header carries total violations · excused · OVERRIDE-RATE % · unexcused plus a per-reason breakdown; override-rate is a tracked red-flag KPI exactly as it is for SLA breaches — a high rate means the target or the population is wrong, not that the team is excused. Purpose: WORK THE TAIL — excuse off-hours and non-support-thread noise so the genuine misses stand alone. Per violation the table shows subject + conversation id (Intercom deep link), customer, triage business-hours primary with wall-clock secondary, FRT (firstSupportReplyFromInbox*), 'answered before Sev' / 'Sev at close' badges, and severity. Same population, window and customer filter as the breach tables (filteredInScope).",
