...
"Listens for assignment webhook topics: conversation.admin.assigned, conversation.admin.open.assigned, ticket.admin.assigned, ticket.team.assigned",
    ...
    },
    {
      ...
      data: {
        ...
        icon: Users,
        details: [
          "Resolver order (first match wins): tag 'enterprise-not-enterprise' → 'not_enterprise' (population gate, short-circuits before override — evaluated ABOVE override so an out-of-scope ticket is excluded even if an override was set) → override → slack_channel (mapped, non-internal) → domain (v3_customer_accounts.domains, excluding lovable.dev + v3_personal_email_domains) → workspace_id (v3_workspace_customer_map, medium confidence) → 'unattributed'.",
          "LOCKSTEP: SQL public.v3_derive_customer (now 6 args: _contact_email, _override_key, _contact_domain, _slack_channel_id_detected, _workspace_id_detected, _tags text[] — old 5-arg / 2-arg overloads dropped) + BEFORE trigger intercom_tickets_v3_apply_customer (passes NEW.tags) + propagate triggers on v3_customer_accounts / v3_channel_account_map / v3_internal_channels / v3_workspace_customer_map + Deno _shared/v3-customer.ts (passes _tags) + backfill_v3_customer_keys (passes each ticket's tags). Editing rules requires updating all of them.",
          ...
          "Verified coverage: 'attributed' = resolved to a registry account. Manually-entered override keys that don't match any account are ORPHANS — surfaced for reconciliation, not counted. v3_coverage_current() computes live numbers and now returns excluded_not_enterprise (Rule 0 count) + population (= total_tickets − excluded_not_enterprise); pct_attributed is computed over population (in-scope tickets), NOT total_tickets. pg_cron 06:00 UTC calls v3_capture_coverage_snapshot() into v3_coverage_snapshots (which gained an excluded_not_enterprise column) for the trend chart.",
          "UI /customers (src/pages/Customers.tsx) — 4 tabs: Coverage (KPI shown as 'N of {population} in-scope tickets', a 'Non-Enterprise (excluded)' KPI card with tooltip, method distribution, trend), Unattributed (grouped queue by domain/channel/workspace/no-signal), Channels (list + auto-suggestions from v3_channel_account_proposals; high = domain co-occurrence, medium = name-token match on registrable domain label), Registry (CRUD on v3_customer_accounts, workspace cache, internal channels; orphan reconciliation with fuzzy suggestions). Rows expand to show underlying tickets via v3_tickets_for_channel / v3_tickets_for_override_key with deep-links to Intercom.",
          ...
          "Design principles: surface problems loudly (unresolved → visible queue, never a guessed bucket); human signals are advisory, stored attribution is system-derived (a bad manual entry can only fail to match, not corrupt data). Ticket-status taxonomy (three-way, carried on the ticket via read-only Intercom tags since status is temporal): not-enterprise (enterprise-not-enterprise tag → excluded from population, Rule 0), prospect (enterprise-prospect tag + real registry account → counted as pre-sales load), customer incl. former (plain registry account, no tag → attributed and counted).",
        ],
        accent: "default",
      },
    },
    {
      ...
      data: {
        ...
        desc: "Measures actual SLA shape (FRT, resolution, reopen, handling time) AND evaluates it against provisional per-severity compliance targets. Per-ticket validation tool + snapshot-based batch reporting. Strictly read-only.",
        icon: Beaker,
        details: [
          ...
          "Multi-clock metrics, each with a Europe/Berlin DST-aware business-hours variant (Mon–Fri 09:00–24:00, 15h/day, BUSINESS_DAY_SECONDS): first response any-agent from open, time-to-escalation + basis, first human reply from escalation, first human reply from open, resolution-active (stop-the-clock), raw TTR (reference only), reopen count, handling time. Escalation anchors on post-AI human handoff, not the +1s routing assignment.",
          "Stop-the-clock resolution (resolutionActiveS / resolutionActiveBusinessHoursS in computeSla): walks open→last-close excluding intervals we're awaiting the customer and closed-then-reopened gaps. Industry-standard ball-in-our-court — we don't breach because a customer is slow (one real ticket: 68 of 76 business-hours were customer/IT wait). evaluateCompliance reads from resolutionActive*, never raw TTR.",
          "First Response for compliance is measured FROM THE AI→HUMAN HANDOFF (firstHumanReplyFromEscalation*) when escalationBasis is post_ai_handoff or marker — else from open (firstHumanReplyFromOpen*). Charging Sam's entire handling window to the human clock was wrong (~15h-from-open vs true ~48min-from-handoff on one real ticket).",
          ...
          "Compliance evaluation (SLA_TARGETS + parseSeverity + evaluateCompliance in slaMetrics.ts, ComplianceSection in SlaTest.tsx): provisional per-severity targets — Sev1 FR 30m/Res 8h wall-clock 24/7 (pending Development off-hours-coverage buy-in); Sev2 FR 4h/Res 2 business-days; Sev3 FR 1 business-day/Res 5 business-days; Sev4 FR 3 business-days/Res best-effort. Severity read from raw_payload.custom_attributes.Severity; parseSeverity returns null for missing/unknown and is NEVER defaulted — missing goes to a visible 'Unclassified' bucket (surface-errors-loudly). SLA_TARGETS is the single source of truth.",
          "UI /sla-test — Tab 1 'Analyze by ID (live)': paste ≤10 ids → sla-ticket-analyze → per-ticket card with headline strip (our human FRT calendar+BH vs Intercom time_to_admin_reply + Δ + Intercom SLA status), color-coded timeline, calendar|BH metric table, origin + flag badges, prominent internal/no-customer warning. Validation / spot-check only.",
          "UI /sla-test — Tab 2 'Batch (stored)': snapshot-based, NOT live. Reads intercom_tickets_v3 filtered by lifecycle_status IN ('finalized','reopened_after_finalize') — full finalized enterprise population, ALL owners (owner filter removed), paged 500. Cleans population (excludes not_enterprise / enterprise-fyi / enterprise-duplicate / merged_ticket / rsa_override=false; rsa_override=true overrides; noCustomer separated). Compliance section groups in-scope by severity + Unclassified, shows N / FR %met / FR breaches / FR n-a / Res %met / Res breaches / Res n-a, plus collapsible First-Response and Resolution breach lists (worst-first). Snapshot-not-live because live-fetching a whole population per page load would hammer the metered Intercom API — snapshot is fast, free, reproducible, as-of-a-time.",
          ...
          "Caveats when reading numbers: OPEN tickets (~10%) excluded from Batch (raw_payload has no conversation_parts in snapshot) → resolution optimistically biased; company holidays not yet modeled; targets are provisional/unratified; duration display renders business-hours with 24h-days (values correct, labels can mislead).",
          "Known open items: Slack-native-then-ticketized escalation semantics; no-customer/CSM own-pool treatment; stored-payload completeness for Slack; bulk-entered Slack tickets; manual initiation-override to correct forwarded-email misclassification; holiday-aware business-hours; aggregate dashboard + SLA compliance slider (plumbing exists, UI wiring after target ratification).",
