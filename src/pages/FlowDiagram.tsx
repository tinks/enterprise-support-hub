    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "New-ticket Slack alert (2026-08-12): when the upsert INSERTS a row (new to our store) AND intercom_created_at is within 24 h, _shared/new-ticket-alert.ts posts one message to Slack channel C0BPDU4JH71 (#enterprise-support-west-coast-float, overridable via env NEW_TICKET_ALERT_CHANNEL) using SLACK_BOT_TOKEN. Dedup is the PRIMARY KEY on public.new_ticket_alerts (intercom_conversation_id) — the claim insert runs BEFORE the post, so a PK collision means 'already announced' and we skip; a Slack failure deletes the claim so a later run retries. The 24 h age gate exists so the daily windowHours=720 wide sweeps can't spam the channel with historical tickets. One alert per ticket, no breach re-nudge. Response carries an alerted counter. Requires the bot to be invited to the channel.",
        ],
