    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "UI /sla-report (SlaReport.tsx, nav 'SLA Report', protected, read-only) — Monthly SLA Report, a DATA-BACKED TARGET PROPOSAL, not a compliance scorecard. Reuses useSlaBatch / computeSla / evaluateCompliance / aggregate / SLA_TARGETS + rowClosedAtMs — no engine, hook or resolver changes. Why framed as a proposal: SLA_TARGETS is explicitly PROVISIONAL, so Median/p90 distributions are the HEADLINE EVIDENCE for where targets should be set and every '% met' is labelled 'vs PROPOSED target' under a top banner ('Proposed SLA targets — performance baseline for calibration, not committed-SLA compliance'). The backlog what-if slider is the live companion; this static report is its precursor. Population = the hook's inScope (all Enterprise finalized/reopened), NO owner filter; month selector (last 12, default current calendar month), membership = finalized-in-month via rowClosedAtMs. Sections: §1 Scope & Population (population count; exclusion breakdown by reason not_enterprise / fyi+duplicate / merged_ticket / rsa_override=false / test_account / prospect_personal / enterprise_prospect; noCustomer + manuallyLogged; severity reconciliation population = Sev1+2+3+4+Unclassified with ✓/✗ MISMATCH, and a LOUD 'Unclassified severity: N (missing Severity attribute — cannot be scored)' block — unscoreable rows stay IN the population, never dropped) · §2 Headline (overall FR %met + Res %met) · §3a/§3b by-severity tables (n · Target+clock · %met · not-evaluable · Avg · Median · p90) · §4 Breaches (FR + Res lists with Intercom deep-links, excused badges, override rate) · §5 By Source (Slack / Sam-first / Direct: n, FR %met, Res %met, Avg+Median, pre-inbox median) · §6 Data-Quality Footer. COUNTING RULES: %met = met_true / (met_true + unexcused breach); excused AND not-evaluable are BOTH excluded from the denominator and shown as their own counts (they are different kinds of non-data-point — folding either in would silently move the headline). Sev 4 Resolution has no committed target → 'no target (best-effort)', no %met, Avg/Median still shown. Clock basis is per severity: Sev 1 calendar/24-7, Sev 2–4 Berlin business hours — so the headline basis is 'each severity's committed clock', never blanket business-hours. Includes the shared 'Show test data' toggle + banner (test-account tickets excluded by default).",
