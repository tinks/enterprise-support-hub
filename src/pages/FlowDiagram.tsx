...
"Listens for assignment webhook topics: conversation.admin.assigned, conversation.admin.open.assigned, ticket.admin.assigned, ticket.team.assigned",
    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "IN-APP SUPPORT FORM FLAG (16 Sep 2026, migration 0047_v3_in_app_form_flag): intercom_tickets_v3 gains is_in_app_form (boolean NOT NULL default false) + in_app_form_source ('tag' | 'attribute' | 'signature') and a partial index on intercom_created_at WHERE is_in_app_form. Detection is ONE shared helper, supabase/functions/_shared/v3-in-app-form.ts (detectInAppForm), called from sync-v3-open (both the search-payload minimal upsert and the full-GET open upsert) and sync-v3-closed's finalize row. PRIORITY: (1) Intercom TAG in {in-app-form, in_app_form, in-app-support-form, app-support-form}; (2) CUSTOM ATTRIBUTE whose key is Intake channel / Source / Request source / Origin with a value reading as 'In-app form'; (3) SIGNATURE on the current template — subject starting '[Lovable support] - ' OR an opening body carrying all three of 'User ID:', 'Workspace:', 'Project:'. Tiers 1-2 exist so the metric survives a redesign of the form copy; tier 3 is the brittle fallback and is deliberately LAST. STICKY BY DESIGN: both columns are only ever written when detection is TRUE (spread-when-true), so a later payload lacking the signal can never silently un-flag a ticket. NO customer-mapping change and NO Project/Workspace parsing — email-domain resolution already attributes every in-app ticket. Backfill (16 Sep 2026, SQL) marked 10 rows created 20 Aug - 11 Sep 2026, all source='signature'. UI: InAppFormUsageCard on Insights > Channels — total, detection-source mix, per-month bars, top accounts, 8 most recent (subject + Intercom id). Deterministic, no AI, no credits.",
          "ACTIVE-TAB ATTENTION BADGE — FALSE-POSITIVE FIX (21 Sep 2026, src/pages/InboxV3.tsx, Option B): the Active tab badge used to fire on last_synced_at older than 30 min. That threshold NEVER matched the sync design — the 5-min open_refresh only revisits tickets with recent Intercom activity, so a quiet ticket's last_synced_at ages by design and the badge reported ~20 of 51 rows as a problem with nothing actually wrong. A permanently-lit alert trains you to ignore it. NEW RULE (anomalies only): state === 'closed' while the row is still lifecycle_status IN (open, reopened_after_finalize) — i.e. the same drift logic the Finalized tab uses, inverted — OR last_synced_at NULL / older than 24 h, which means the row missed BOTH twice-daily 30-day wide sweeps (06:15 and 18:15 UTC) and has genuinely dropped out of sync. Routine cadence gaps stay silent. Verified against live data at the time of the change: 51 active rows, 0 closed-state drift, 0 drop-outs — badge correctly hidden. The Finalized tab rule is unchanged.",
        ],
    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "Topic deep dive tab (src/pages/search/TopicDeepDive.tsx -> analyze-topic-trends): retrieval + clustering, not lookup. Full-text over esh_search_index kind=v3_ticket bounded by meta.created_at (7-365d, default 90), <=600 retrieved / 120 sent to openai/gpt-6-astra via one streamed /v1/responses call with a strict json_schema (summary + 4-8 themes: name, description, category bug|support_gap|feature_request, recommendation, ticket_ids). Deliberately text-derived rather than grouped by Affected product area: 69 GitHub tickets in 90 days but only 26 carried the GitHub Integration area. Editor-gated, writes nothing (no Intercom, no columns, no index); model ticket ids are intersected with the ids actually sent so a hallucinated id can never surface. UI adds a client-computed volume-by-month strip, expandable evidence lists and a Slack mrkdwn digest. Verified 11 Sep 2026: 'GitHub' / 90d -> 64 matched, 8 themes, monthly 3/16/27/18. UNVERIFIED: the >120-ticket truncation path and the 402/429 error branches.",
        ],
        accent: "blue",
      },
    },
    {
      ...
      data: {
        label: "Subject labels: human override + AI subject",
        desc: "Intercom titles like \"Intercom #2154748...\" carry no meaning. The Hub holds its own descriptive label — typed by a human, or written by AI when Intercom gave a placeholder. Display only — nothing is written to Intercom, and no SLA/triage measurement changes.",
        icon: ClipboardList,
        details: [
          "Storage: intercom_tickets_v3.subject_override (+ _by / _at) for the human label, and subject_ai (+ subject_ai_at / subject_ai_model / subject_ai_source_hash) for the AI one. Intercom's own subject column is never touched, so either label can always be reverted. _shared/v3-finalize.ts writes only subject, so re-sync and finalize cannot clobber them.",
          "One display rule, in src/lib/subjectDisplay.ts: displaySubject(row) = subject_override → subject_ai → subject → \"Untitled\". The human label always beats the AI one. Read by Triage, Inbox v3, Prospects, Dev escalations, SLA workbench, Owner dashboards v3, Resolution anatomy, CSAT report, Monthly lookback and the Customer report. Overridden rows show an 'edited · Intercom: <original>' subline, AI rows an 'AI · Intercom: <original>' one — a label never hides what Intercom says.",
          "AI writer: supabase/functions/generate-ticket-subject (model openai/gpt-6-astra, /v1/responses, streamed). mode=auto picks OPEN tickets whose Intercom subject matches the shared placeholder rule (^Intercom #\\d+$, blank, (no subject), untitled, -, n/a) and skips any ticket with a human override or a usable subject; mode=manual takes one conversation id and works on ANY ticket. Closed tickets are never rewritten, by either path. Cost discipline: kill switch settings.subject_ai_enabled, daily cap settings.subject_ai_daily_call_cap (default 200, counted off subject_ai_at), max 25 ids per call, 4000-char input truncation, and a content hash (subject_ai_source_hash) so an unchanged ticket costs zero model calls.",
          "Editing: inline pencil on the Subject cell of every v3 table (editors only) with a Clear action, plus Subject field, 'Rewrite with AI' and \"Use Intercom's\" in TicketFieldsPanel, kept visually separate from the Intercom-mirrored fields. These writes do NOT go through esh-write-action — Hub-local by design, gated by 'Editors update intercom_tickets_v3' (can_edit(auth.uid())); read-only roles get 'Update refused — editor role required.'",
          "Audit: subject_override_set / subject_override_cleared / subject_ai_written append to conversation_audit_logs with old + new values and the actor.",
          "Trigger: no HTTP cron (blocked on this project). Instead sync-v3-open runs a tail hop at the end of its 5-min pass — only when that pass inserted or reopened a ticket AND an open placeholder with no AI/human label is actually pending — invoking generate-ticket-subject with {mode:auto, limit:10} as service role. Best-effort: a failed hop logs and never fails the sync. Both branches log ('subject-ai hop pending=N [status]' / 'skipped — no open placeholders pending').",
          "Verified 24 Aug 2026 on Intercom #215474865211089: human label rendered, edited and cleared, both actions logged. Verified 9 Sep 2026 for the AI writer: #215475214997973 titled 'Lovable app backend migration from EU to US'; a re-run with the same content made 0 model calls (hash skip); the kill switch returned 403; unauthenticated and bad-cron-secret calls returned 401; backfill wrote 19 open placeholder tickets and left the 263 closed placeholders untouched. UNVERIFIED: the read-only refusal path, the daily-cap refusal, and the sync-v3-open tail hop firing live (deployed 9 Sep 2026 with 0 placeholders pending, so it has not yet had work to do).",
    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "Channel + prompt are configuration, not code: settings.pax_help_channel_id (or env PAX_HELP_CHANNEL_ID) and settings.pax_request_template with {url} / {id} / {subject} / {pax} placeholders. With neither set, the channel is resolved by name (#pax-ets-help) via conversations.list.",
          "Real Slack mention required: plain-text '@Pax' is inert and never wakes the bot. Pax's member id resolves from PAX_SLACK_USER_ID -> settings.pax_slack_user_id -> a users.list scan (result written back to settings), and is rendered as <@Uxxxx> with link_names: true. Literal '@Pax' in a stored template is rewritten to the mention. If no Pax user resolves, the request is BLOCKED rather than posting a message Pax cannot see.",
          "UNPAUSED, now posts AS THE HUMAN (10 Sep 2026): Pax rejected the shared Ask Lovable bot identity because it is not a verified lovable.dev member. Every Slack call in ask-pax-investigate now runs through the acting teammate's OWN Slack authorisation (Slack App User Connector, user scopes chat:write / channels:read / users:read) via the Lovable connector gateway. There is deliberately NO shared-bot fallback: with no personal connection the request is refused 409 with slackConnectRequired.",
          "Per-person authorisation: slack-user-connect starts OAuth in a popup, /oauth/slack/return forwards only a one-time code, slack-user-connect-complete exchanges it and stores the lovack_* key AES-GCM encrypted in public.app_user_connections (service-role only, deny-all RLS for anon/authenticated). slack-user-connection reports status and disconnects. The browser never sees a connection key or a Slack token.",
          "Roster gate: connect and Ask Pax are restricted to active public.teammates rows with role = 'support' (the Enterprise Support team). A gateway 401 of type credential_* is surfaced as 'reconnect needed', not as a generic error.",
          "VERIFIED LIVE (10 Sep 2026) on conversation 215475881886059: OAuth consent completed, the #pax-ets-help post was authored by the human account (permalink p1789055344830919), Pax replied and investigated, note_state = 'linked' with the Intercom internal note attributed to admin id 10765619, and esh_ticket_actions logged ask_pax_investigate = succeeded. UNVERIFIED: a repeat server-side mode='start' call in the human flow, reconnect after expiry, disconnect, the 409 slackConnectRequired refusal, the kill-switch-off refusal, and the note-failure Retry path.",
        ],
        accent: "blue",
      },
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "VERIFIED (23 Sep 2026): 215476053149547 returned 200 with grounded user/project/workspace IDs, status customer_reported_unverified, severity workaround_available kept separate from the customer's 'showstopper' quote; bad id returned 400. UNVERIFIED: read-only 403, Intercom 404, AI 402/429 paths.",
          "REVIEW PANEL (23 Sep 2026): the ticket sheet's Investigation card has TWO buttons (fix 23 Sep): 'Ask Pax to investigate' (primary, one-click, original direct start) and 'Draft Dev Escalation' (outline) → extract-human-info → HumanInfoReviewPanel inline. Editable summary / customer statement (visible 'Review for sensitive info' reminder) / IDs with one-click candidate pills / expected-observed / evidence / steps; impact badge beside the customer's urgency quote. Every field carries its 🟢/🟡/🔴 provenance tag into the posted text.",
          "SUBMIT GATE (src/lib/humanInfo.ts reviewBlockers): disabled until the reviewer deliberately clicks a repro status (the pre-fill is shown as '(suggested)' but does not count); 'Could not reproduce (infra limitation)' requires a reason; summary required. assembleHumanInfo re-runs the same validation and throws, so the gate is not UI-only.",
          "POST: ask-pax-investigate accepts optional humanInfoBlock (1–3500 chars, must start '*Human Info'), prepended ABOVE the unchanged Pax request in #pax-ets-help; Pax template, idempotency, Intercom note unchanged. Audit payload records human_info true/false. Summary carries insufficient_data: when the thread lacks a concrete component+failure the value is blank and the panel says 'Not enough detail to draft' instead of filler.",
        ],
        accent: "blue",
      },
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "DEV ESCALATION SPLIT (11 Sep 2026): My Queue now LIVE-JOINS public.dev_escalations by intercom_conversation_id (the same Linear mirror /escalations reads) instead of relying on eng_wait_start_at / eng_wait_end_at, which are finalize-time fields and are NULL on most open tickets — that gap is why CLO-1225 (Intercom 215475556254768) showed as 'Ready for follow-up' while it was actually blocked on engineering. Two new buckets sit directly under Action needed: 'Waiting on dev' (linear_state_type backlog/triage/unstarted/started) and 'Dev resolved' (completed/canceled while the Intercom ticket is still open). A ticket where the customer spoke last stays in Action needed — replying to the customer outranks chasing dev. Once the fix is acknowledged the ticket falls back to the ordinary conversational buckets.",
          "FOLLOW-UP CADENCE (src/lib/devEscalation.ts, Hub-owned, migration 0043): dev_escalations gains dev_next_followup_at / dev_followup_source / dev_fix_ack_at / dev_fix_ack_by, all nullable and never synced from Linear. Lifecycle defaults: 24h while the escalation is pre-ack (backlog/triage/unstarted or unassigned), 3 days once it is in flight; due date = dev_next_followup_at when set, else (dev_followed_up_at ?? created_at) + default. Overdue rows show 'Chase due', feed the 'X need a chase' sub-count on the Waiting-on-dev card and the 'Chase dev' filter. Detail sheet (editor-gated via useCanEdit) offers 'Mark followed up' (applies the lifecycle default) plus manual overrides +1d / +3d / +1w / custom date, and 'Acknowledge resolution' / Undo for resolved escalations, stamped with the signed-in email. COPY UPDATE 18 Sep 2026: the button reads 'Acknowledge resolution' (was 'Acknowledge dev fix') and the helper text is cancel-aware — a canceled Linear issue reads: Engineering canceled this issue (wont fix) — because Dev resolved covers both completed and canceled states (e.g. CA-2345 canceled on 215475904659797). Label only; no behaviour, column or write change. NOTHING is written to Intercom, Linear or any reporting column — Dev-resolved exit is DELIBERATELY MANUAL, no auto-clearing from Intercom activity.",
          "WORKAROUND PROVIDED OVERRIDE (11 Sep 2026, migration 0044): dev_escalations gains dev_workaround_at / dev_workaround_by / dev_workaround_note, Hub-owned and nullable. When dev hands over a manual workaround while the Linear issue sits in the backlog, a human records it: hasWorkaround() makes needsChase() false and MyQueue drops the ticket out of Waiting on dev back into the ordinary conversational buckets, while the Linear escalation card stays visible with live Linear state. isDevDone STILL WINS — a completed/canceled issue resurfaces as Dev resolved with 'Acknowledge resolution' even with the flag set. Clearing restores the normal cadence. Editor-gated, no Intercom or Linear write. Verified live 11 Sep 2026: CLO-1074 moved Waiting on dev → Ready for follow-up (Chase dev 1→0); SCA-3522 (Done) stayed Dev resolved; both flags cleared afterwards.",
          "DETAIL SHEET REDESIGN (11 Sep 2026, presentation only): IssueDetailSheet now shows the Intercom conversation ID in its header as a mono chip with a copy button beside 'Open in Intercom' — it was previously not shown anywhere in the panel. New optional props 'wide' (640/760/860px at sm/lg/xl vs the fixed 480px) and 'raw' (skip the <dl> wrapper); every other caller keeps the old 480px behaviour. My Queue uses wide+raw and groups the body into four bordered cards — status (badges + why + 3-col metadata grid via the new IssueStat), engineering escalation, subject & classification (TicketFieldsPanel), investigation (PaxInvestigateControl). TicketFieldsPanel's subject action row now wraps instead of overflowing, which is what previously clipped 'Use Intercom's'. No query, write path, bucket rule or cadence changed.",
          "SHARED TICKET PANEL (11 Sep 2026, src/components/issues/TicketDetailContent.tsx): ONE ticket panel intended for every surface — My Queue today, Inbox V3 / Escalations next. Collapsed by default: caller-supplied state pills + Sev/Owner/Area/Type chips + a READ-ONLY 'Esc to eng' chip mirroring Intercom's 'Escalated to Engineering' attribute, a one-line why blurb, metadata grid, engineering card, message cards, Pax investigation. A pencil expands the existing TicketFieldsPanel inline, so every write still goes through esh-write-action with UNCHANGED contracts and nothing new is written to Intercom. 'Escalated to Engineering' write-back was DELIBERATELY NOT WIRED (setting it in Intercom exposes a further link field); esh_write_allowed_actions unchanged. OPTION A for a missing escalation: the engineering card is a caller-filled slot, OMITTED ENTIRELY when the ticket has no dev_escalations row — no empty shell. MESSAGE CARDS (src/lib/ticketComments.ts + TicketCommentCard.tsx) read the already-persisted raw_payload — no new fetch, no Intercom call: Initial message from source.body, Latest reply = last conversation_parts entry with part_type='comment' and a non-empty body (internal notes and state changes can never surface as a reply). HTML stripped, author/timestamp extracted, preview capped at COMMENT_CAP=280 with Show more/less; the latest card is hidden when it duplicates the initial. Verified live at 2000px including the no-escalation case.",
          "LATEST INTERNAL NOTE CARD + HUB SNOOZE (11 Sep 2026, migration 0045 v3_ticket_snooze): ticketComments() also returns 'note' — the newest conversation_parts entry with part_type='note' and a non-empty stripped body — rendered as an amber 'Latest internal note' card below Latest reply. isAutomatedNote() suppresses the Hub's own bookkeeping notes (author matching /lovable support/i, or a body containing slack.com/archives/). Customer-facing extraction is unchanged and no note advances last_admin_reply_at or any SLA clock. SNOOZE: intercom_tickets_v3 gains nullable snoozed_until / snoozed_at / snoozed_by / snooze_reason plus a partial index — Hub-owned, never synced from or written to Intercom, never read by sla-core or reporting. A row with snoozed_until > now() KEEPS ITS NATURAL BUCKET but sorts below every unsnoozed row, dims to opacity-50 and shows 'Snoozed until d MMM'; a ninth top-level 'Snoozed' stat card counts and filters them. Expiry needs no cron — the row simply stops being snoozed on the next load. Controls sit in TicketDetailContent's new 'extra' slot, editor-gated by useCanEdit: optional reason, quick picks +1d/+3d/+1w (CADENCE_CHIPS), Custom date popover, Wake now. Verified live 11 Sep 2026 on 215475781288266: note card renders Matt's 8 Sep note while Latest reply keeps Diana's 4 Sep message; +1w set 18 Sep, Snoozed 0→1, row dimmed and sank while staying Action needed; Wake now cleared all four columns.",
          "SNOOZE AUTO-WAKE (16 Sep 2026): a snooze holds until its date UNLESS reality moves — a customer reply after the snooze was taken, or engineering resolving (completed/canceled) the linked Linear issue after the snooze was taken. Both compare against snoozed_at with a 1s clock-skew buffer, so a deliberate re-snooze taken after the event is honoured; nothing else wakes a ticket. TIER 1 display-side in MyQueue.classify: QueueRow.autoWoke = 'customer_reply' | 'dev_resolved' | null and snoozed = snoozeDated && !autoWoke, so the row un-dims and leaves the Snoozed card/filter on the next load with no sync lag; dev_resolved requires needsFixAck(esc) and uses the new devResolvedAtMs() helper over the linear_completed_at / linear_canceled_at mirror (now selected by My Queue). The snooze card shows an amber 'Woken automatically — …' line plus Clear snooze. TIER 2 durable: sync-v3-open clears all four snooze columns (same shape as Wake now) when statistics.last_contact_reply_at beats snoozed_at, on BOTH the full-fetch and the cheap minimal search-payload path, reported as snoozes_woken; sync-linear-escalations does the same after its escalation upsert for issues resolved after snoozed_at, also reported as snoozes_woken. No Intercom or Linear writes, no SLA or reported-number impact. Verified live 16 Sep 2026: 215475949159138 showed the woken card and sync-v3-open (windowHours 72) returned snoozes_woken 2 with its columns now NULL. UNVERIFIED: the Linear-resolution wake path.",
        ],
        accent: "blue",
      },
    },
    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "CONTEXT SPLIT (9 Sep 2026): the context, hook and types live in src/lib/actionSignalsContext.ts; src/hooks/useActionSignals.tsx exports ONLY the provider component. Reason: a module exporting both a component and non-component values breaks React Fast Refresh, which re-evaluated the module and created a SECOND context — consumers then read the fallback and the page reported '0 signals watched · 0 need attention' while every loader was fine. Verified after the split: 14 signals watched, 1 needs attention.",
          ...
          "DOC DRIFT SIGNAL (11 Sep 2026): doc_drift compares the repo file /.lovable/project-knowledge.md against BOTH knowledge_documents.content and pending_content (src/lib/knowledgeDrift.ts, read-only, 5 s shared cache). doc_approvals alone could not see documentation edited on disk but never staged, so the safety net read 'clear' while docs drifted. HTML responses throw (unreadable card) instead of faking drift. Verified live: 15 signals, drift amber at +7,926 chars.",
          "AUTH GATE (14 Sep 2026): ActionSignalsProvider wraps the whole router including /login, so a signed-out page load fired every loader as anon and two SECURITY DEFINER RPCs granted to authenticated only (v3_unattributed_groups, v3_channel_proposals_pending) logged recurring 'permission denied for function' errors. The provider now tracks the session (onAuthStateChange + getSession) and load() returns early until signed in; the focus/visibility refetch inherits the guard. Migration 0046 makes the authenticated GRANT EXECUTE explicit and REVOKEs anon on both functions so a later CREATE OR REPLACE cannot silently widen access. Signed-in behaviour, counts and mutes unchanged.",
        ],
        accent: "default",
      },
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "EXPLICIT DENY-ALL ON INTERNAL TABLES (9 Sep 2026, migration 0037): public.cron_auth and public.gmail_oauth_states each gained a RESTRICTIVE 'FOR ALL TO anon, authenticated USING (false) WITH CHECK (false)' policy plus a service-role-only table comment. CORRECTION to the Batch B text above: both tables actually DO hold full arwdDxtm grants for anon and authenticated (pg_class.relacl verified) — the only thing blocking those roles was RLS-on-with-zero-policies, i.e. default deny. Real, but fragile: one permissive policy added later would have opened the table. A restrictive policy can never be widened by a permissive one, so the denial is now unconditional. Service-role (all edge functions, esh_cron_headers()) is unaffected — it bypasses RLS. VERIFIED: pg_policies shows exactly the two new RESTRICTIVE policies; linter findings 32 → 30 with the rls_enabled_no_policy category cleared. PARKED: revoking the redundant anon/authenticated grants, moving pg_trgm out of public, and the 29 authenticated-executable SECURITY DEFINER functions.",
          "STORAGE OBJECT LOCKDOWN (11 Sep 2026, migration 0045): the three policies named 'Deny public insert/update/delete public-assets' were PERMISSIVE with an inverted condition (bucket_id <> 'public-assets'). A permissive policy GRANTS, so they granted INSERT/UPDATE/DELETE on every bucket EXCEPT public-assets to anon and authenticated — including the private customer-attachments bucket holding the 70 re-hosted Slack files. Anyone with the publishable anon key could have written or deleted customer attachments. All three dropped and replaced with explicit allows: public_assets_read (SELECT, anon+authenticated, public-assets only) and public_assets_admin_insert/update/delete (authenticated AND has_role(auth.uid(),'admin'), public-assets only). customer-attachments now has NO storage.objects policy at all — deny by default; reads keep working solely because the `attachment` edge function mints 7-day signed URLs with the service role, which bypasses RLS. VERIFIED with the live anon key: upload to customer-attachments 403 'new row violates row-level security policy', upload to public-assets 403, delete of public-assets/lovable-logo.png 403, public read of the logo 200, public read of knowledge-sync/project-knowledge.md 200. UNVERIFIED: an admin-session upload to public-assets (nothing in src/ writes to storage from the client). STILL OPEN, backlogged as 'Define role-based data visibility (Enterprise Support vs everyone else)': the 55 tables whose read policies are USING (true) — any of the ~14 active Hub accounts can read every row, though only the 5-person Enterprise Support team should. Not a quick swap: a hub_members predicate changes nothing for those accounts, narrowing to 5 blanks CSM pages with no defined replacement scope, and ~29 SECURITY DEFINER routines bypass table RLS anyway.",
          "THREE CRITICAL FINDINGS CLOSED (21 Sep 2026, migration 0050): (1) sla-ticket-analyze now gates on requireEditor instead of requireUser — the 'Analyze by ID (live)' proxy fetches arbitrary Intercom conversations with the server token, so read-only accounts can no longer reach it; the caller-supplied ID list is deliberately NOT restricted to rows already in intercom_tickets_v3, because fresh and out-of-inbox tickets are the normal use case. (2) sam_ticket_reviews SELECT policy 'Authenticated can read sam reviews' (USING (true)) dropped and replaced with 'Editors can read sam reviews' USING public.can_edit(auth.uid()) — reads now match the existing insert/update/delete rule. (3) Slack attachment re-hosting validates before storing: shared rejectAttachment(name, mimetype) in _shared/attachments.ts enforces a support-safe extension allowlist (images, video/audio, logs/HAR/JSON/CSV/PDF/Office, archives), an explicit executable/script blocklist, and a block on application/x-* or *executable* content types; the DOWNLOADED blob.size is re-checked against ATTACHMENT_MAX_BYTES (50 MB) after fetch, not just Slack's reported size. Applied identically in slack-events and slack-interactions. Rejections are skipped with a logged reason; no other behaviour changed.",
          "SCAN FOLLOW-UP — LAST TWO CRITICALS CLOSED (21 Sep 2026): the scanner re-flagged two items because it analyses each function file in isolation and cannot see validation behind an import or infer caller scope. (1) sla-ticket-analyze now resolves every normalized id against the Hub BEFORE calling Intercom: a service-role client checks intercom_tickets_v3.intercom_conversation_id and conversation_mappings.intercom_conversation_id; unknown ids return a per-id 404 'Ticket not found in synced ESH records' instead of being proxied, and a failed lookup returns 500 scope_check_failed rather than falling open. requireEditor still applies. Accepted tradeoff: a ticket created in Intercom in the last ~5 minutes cannot be analysed until sync-v3-open picks it up — SLA analysis is retroactive, so the window does not matter. This REVERSES the earlier deliberate decision not to constrain the id list. (2) The attachment allowlist, executable/script blocklist, MIME block and 50 MB pre/post-download size checks are now declared INLINE in slack-events/index.ts and slack-interactions/index.ts instead of imported from _shared/attachments.ts (which keeps ATTACHMENT_BUCKET / attachmentUrl / ATTACHMENT_PREFIX). Rules are byte-identical in both files — change one, change the other.",
        ],
        accent: "default",
      },
    },
    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "CONSOLIDATION (21 Sep 2026): Admin → People (/people, src/pages/People.tsx) is now the SINGLE people-management pane and the only one in the nav. /users no longer renders UsersCard — App.tsx redirects /users → /people (UsersCard.tsx and Users.tsx kept on disk as rollback, unreferenced). People keeps its joined access+attribution row model and adds every write that previously lived elsewhere: 'Add person' dialog (name, @lovable.dev email, team, Slack ID, Intercom Admin ID, optional dashboard entry, optional Hub login which inserts hub_members pending + invokes hub-access-manage provision, then grants editor/admin on the returned user_id); Provision / Remove access / Unblock in the access cell; editor + admin checkboxes per row with the last-admin checkbox disabled and a tooltip; inline pencil editing of intercom_admin_id and slack_user_id; a show_dashboard switch. Any write touching intercom_admin_id re-mirrors the FULL teammates roster (active AND inactive) into settings.admin_owner_map, which intercom-webhook, poll-intercom-inbox, sync-v3-open/closed, reconcile-v3-open, backfill-enterprise-inbox, esh-write-action and AnalyticsV3 still read. Saving a Slack ID (or adding a person with one) also stamps resolved_at on matching open relay_attribution_gaps rows, so the Action Center identity-gap card clears without SQL.",
          "VERIFICATION (21 Sep 2026): typecheck + build clean; Alex K (alexandra.kosovic@lovable.dev, Slack U0AQWBY6TU3, role csm) added to teammates and her relay_attribution_gaps row resolved — 0 open gaps remain. UNVERIFIED in the browser: the Add-person dialog end to end, the provision branch, role checkboxes, inline ID save and the dashboard switch (no authenticated browser session available this pass).",
          "INTENTIONAL NO-LOGIN (21 Sep 2026): migration 0049_teammates_hub_access_expected adds teammates.hub_access_expected BOOLEAN NOT NULL DEFAULT true. The 'No Hub login' drift flag now fires only when `!hasAccess && teammate && role !== 'ai' && hub_access_expected` — so Sam (role ai) is skipped automatically and forever, and any human can be marked with a per-row 'Internal, no Hub login' checkbox in the ESH access cell (renamed from 'attribution only' on 21 Sep 2026; writes hub_access_expected = false; grants/revokes nothing). The access cell renders 'no login (AI)' / 'Internal, no login' / 'no login' accordingly, and every row with an @lovable.dev address and no account gets a 'Grant access' button that inserts the hub_members pending row (duplicate-tolerant) then invokes hub-access-manage provision — so Alex K and future CSMs can be given ESH access without SQL. Add person sets hub_access_expected = fLogin, i.e. leaving 'Create a Hub login' unticked records the person as internal, no Hub login rather than as drift.",
        ],
        accent: "default",
      },
    },
      ...
      data: {
        ...
        details: [
         ...
         "THREE-WAY SPLIT (engine v2, 1 Sep 2026): customer wait is no longer a residual. resolution_customer_wait_s / _bh_s and resolution_window_s are persisted alongside active and closed, all four walked from the SAME timeline in sla-core.ts, so active + closed + customer_wait = window BY CONSTRUCTION — any drift is an engine bug, not a source mismatch. Raw (Intercom time_to_last_close) stays outside the identity as reconciliation only.",
          ...
          "MOVED LINEAR ISSUES (14 Sep 2026, sync-linear-escalations only — no schema change): keys are now looked up by IDENTIFIER first (issue(id:\"EVERY-2655\")), which Linear resolves to the issue's CURRENT home even after Dev moves it to another team; the old team+number filter (issues(filter:{team,number})) is kept ONLY as a fallback and can never follow a move, which is why five references silently resolved to nothing. The mirror is written under the CURRENT identifier (linear_key = issue.identifier), so a moved issue stops looking un-synced, and the run response returns a moved[] list of old -> new keys. VERIFIED 14 Sep 2026 on a live run: 62 candidates / 55 distinct keys / 55 resolved / 0 not found (was 5 not found), moved = SCA-2893->ENT-3100, INTX-1634->SEC-843, EVERY-2655->FOU-2, CLO-1259->SCA-3592, AGE-993->EVERY-2499; Intercom 215475543627751 now carries FOU-2 (Backlog) in dev_escalations where it previously had no row at all. Old link rows keyed by the retired identifier are left in place (not deleted) — UNVERIFIED whether any stale duplicate affects the eng-wait union.",
          ...
          "ENG-WAIT WINDOW ON /escalations (4 Sep 2026, presentation only): the Eng wait column and the row detail sheet now show the WINDOW behind the number — eng_wait_start_at -> eng_wait_end_at, with eng_wait_source in the hover title — so a reader can check the seconds against the dates instead of trusting them. Rendered only when the engine stamped a start: no start reads 'Window not recorded', no end reads 'still open at close', and neither is inferred from ticket dates. VERIFIED 4 Sep 2026 at 2560px on 215475084466535 (34d 5h, 20 Jul -> 4 Sep, attribute_event). 37 rows carry a window and all 37 have an end, so the 'still open at close' branch is UNVERIFIED.",
          "ENVELOPE vs PER-ISSUE + NOTE-ONLY DETECTOR (4 Sep 2026, presentation only — no schema, no engine, no clock change): the persisted span is now LABELLED 'Envelope:' because on a multi-issue ticket it is the outer first-start -> last-end and is WIDER than resolution_eng_wait_s, which was already exact (union of per-issue windows intersected with customer-wait time). The row detail additionally lists every mirrored dev_escalation_links issue with its own Linear lifespan (created -> completed / canceled / still open, plus state), so the reader sees the parts and the total without the UI re-deriving a second, divergent number — per-issue SECONDS are deliberately NOT shown, since overlapping windows cannot be split per issue without double-counting. DETECTOR: the board extracts /[A-Z][A-Z0-9]{1,5}-\\d{1,6}/ keys from the notes it already loads and subtracts the clock-bearing set (link keys + attribute/override resolution); leftovers surface as an amber 'Note-only Linear keys' field and a banner with a 'Show only these' filter, stated as advisory — a note NEVER moves a clock until a human copies the key into the Escalated Issue attribute or the Hub override. VERIFIED 4 Sep 2026 at 2560px on 215475479744265 (ENT-3478 14 Aug -> 19 Aug Done, ENT-3798 1 Sep -> still open In Review, envelope 14 Aug 18:53 -> 3 Sep 17:09, eng wait 6d 19h) and, for the detector, with a temporary ENT-9999 note that was deleted after the check. Live note-only population is 0 rows today, so real-data firing is UNVERIFIED.",
          ...
          "ENGINE v4 — TIMELINE CLOSE FALLBACK + OUTBOUND BALL OWNERSHIP (9 Sep 2026, ACTIVE_CLOCK_ENGINE_VERSION 3 -> 4): the last 2 not-computable rows are gone, and they were an engine gap, not bad tickets. (1) Intercom returned an EMPTY statistics block on both, and resolveCloseAt() read close time only from statistics.last_close_at / first_close_at — no close time, so every clock stayed null. resolveCloseAt(stats, timeline) now falls back to the LAST close part on the timeline; statistics stay authoritative when present and no timestamp is ever invented. (2) isOutboundInitiated() (source.author.type = admin or bot) means WE opened the conversation, so the ball starts on the CUSTOMER side: computeResolutionActive and customerWaitSegments take initialBallWithUs and open the waiting-on-customer stretch at the clock start. Previously an unanswered outbound email billed the whole silence to us as ACTIVE time — measuring our own outreach as support work. Inbound conversations are untouched (default stays true). Migration 0036 adds outbound_initiated and customer_replied to intercom_tickets_v3, stamped at finalize by activeClockFields() and by backfill-v3-active-clock; outbound_initiated AND NOT customer_replied is the outbound-only / no customer response case.",
          "VERIFIED 9 Sep 2026 on a forced re-backfill of all 656 finalized / reopened rows (500 + 156, failed 0, skipped_no_parts 0, remaining 0): uncomputable 0 (was 2), identity violations 0, outbound_initiated 3, outbound-only 2. 215474972075677 'Connect GitHub repository' — window 475,203s, active 0, all customer wait (we emailed, they never replied). 215475476004105 — window 604,305s, all customer wait. 215475789771012 — outbound but the customer DID reply: active 297,679s / customer wait 137,929s. UNVERIFIED: the pre-backfill values of that third row were not snapshotted, so its delta from the ownership change is not quantified; and 215475476004105 is an internal allowlist request Matt proposed classifying Enterprise FYI and excluding from issue-based SLA/resolution reporting — NOT applied, awaiting his call.",
    ...
    },
    {
      id: "incident-feed",
      type: "flowNode",
      position: { x: COL_W * -1.6, y: ROW_H * 3.6 },
      data: {
        label: "Incident feed (Slack #incidents)",
        desc: "poll-slack-incidents reads incident.io announcements from Slack C07TMQ5E6SC into public.incidents; live banner + /incidents log.",
        icon: Siren,
        details: [
          "Slack-only BY DECISION: the incident.io API is NOT used. A workspace incident.io connector exists but is owned by two other people — it stays backlog until they explicitly grant permission.",
          "incident.io EDITS its announcement in place instead of posting updates, so ingestion is a rolling window (default 2 days) + upsert on unique incident_number. Re-reading a window is free and always converges on the current card. resolved_at keeps the FIRST observed value. Modes: { lookbackDays } | { full: true } | { dryRun: true }.",
          "Three announcement formats are parsed: (1) current — header = title, '*Status*:' section = lifecycle, '[Severity]' prefix; (2) terminal/older — the card is REPLACED, header becomes 'Incident declined/closed/resolved/merged' and the real title moves to the quoted line (without this, 280 rows were titled 'Incident declined' with null status); some cards append severity to status ('Documenting (Minor)') and are split; (3) pre-Oct 2025 — no status line at all, only a header emoji. ONLY :white_check_mark: is mapped (→ Closed); every other emoji stays 'unknown' rather than being guessed into a live/closed bucket.",
          "Customer impact is DERIVED: presence of a statuspage.incident.io public status-page link in the announcement. The API's status-page endpoints 404, so this is the only signal available and it is labelled as derived in the UI.",
          "Surfaces: IncidentBanner in AppLayout (60s refresh, links to incident/status page/Slack channel, session-only dismissal that reopens when the live set changes, and SILENT on read failure rather than a false all-clear) + /incidents historical log (search, severity/category/customer-impact filters DEFAULTING TO status=live so the page opens on 'what's broken now' — 'All statuses' is one click away, sortable headers, CSV export). Storage: standalone public.incidents (migration 0033) — no v3 table or trigger touched.",
          "Health: integration_health.slack_incidents_poll, 30-min staleness, registered in all three lists.",
          "Verified 3 Sep 2026 (full rescan): 2,026 messages scanned, 1,049 announcements parsed, 964 skipped, 1,049 upserted, 0 errors → 1,008 closed / 10 live / 6 post-incident / 25 unknown. The 25 unknown are all Jan–Feb 2025 (earliest card format, no status signal) and are left honestly unknown.",
          "CRON (4 Sep 2026): pg_cron job poll_slack_incidents_15min, '*/15 * * * *' (96 runs/day), net.http_post → poll-slack-incidents with public.esh_cron_headers() and body {\"mode\":\"rolling\"}, 60s timeout. 15 min chosen over 5 min on purpose: frequent polling keeps the DB awake with no work to do, and 15 min of banner lag is inside human reaction time. Authored BY HAND in the SQL editor (agent SQL + migration tool both refuse HTTP cron here) — so it lives in the database only, with NO migration file. First fire verified 21:45:06 UTC: health ok, 0 failures, 7 rows re-stamped, 1,049 → 1,050 incidents. UNVERIFIED: cron.job / cron.job_run_details are permission-denied to every agent role, so the schedule row and multi-fire recurrence were never read back directly.",
          "STUCK-LIVE FIX (9 Sep 2026): Slack's 'oldest' filter matches a message's ORIGINAL post time, not its last edit — so an incident declared before the rolling window and resolved inside it was never re-read and sat 'live' forever (INC-1899/1901/1904/1913, 3–5 days stale). Step 2b now re-fetches EACH still-live/unknown row (newest 60, RECHECK_CAP) by its own stored slack_message_ts and merges it into the same upsert. Two real failure modes fixed with it: Slack returns nothing when oldest === latest even with inclusive=true (bracket ts ± 1s, match exact ts), and terminal 'merged/declined' cards carry the incident link ONLY as an inline mrkdwn <url|label> with no button block — the parser harvested only node.url, so every such card was silently dropped as 'not an announcement' (this is what kept merged INC-1913 live). Summaries report recheck_fetched / recheck_changed / recheck_missing; unparseable re-checks log a warning instead of being swallowed. Verified: recheck_changed 1, recheck_missing [], INC-1913 → Merged/closed (resolved_at 6 Sep 13:02Z), live 15 → 13; full rescan with the new parser recovered 171 previously-unparseable terminal cards (1,049 → 1,228 parsed) → 1,182 closed / 13 live / 8 post-incident / 25 unknown. UNVERIFIED: no cron fire observed since the change — exercised by manual invocation only.",
        ],
        accent: "default",
      },
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "Blank-domain dismissal (8 Sep 2026): a Slack deal with an empty Company Domain marks the run unhealthy until the company is handled. The registry lookup only clears it on an EXACT account_key match, which real registries break (Slack 'Cars Commerce' → cars_commerce; registry holds cars_commerce_us and cars_commerce_canada), leaving an alert that could only be cleared by inventing a third, attribution-affecting registry row. Second clearing path: public.v3_closed_won_acknowledged_names (name_key PK = same slug, display_name, note, acknowledged_by/_email; RLS authenticated read, can_edit() insert/delete). The poller unions acknowledged keys into the handled set → missing_domain_handled[]. Kept as a SEPARATE table, not an alias column on v3_customer_accounts, so acknowledging an alert can never change customer attribution. Settings → Integration health parses the 'blank Company Domain for \"<name>\"' phrase and renders a per-company Dismiss button that inserts the acknowledgement then re-runs the poller, so the card recomputes from a real run. VERIFIED 8 Sep 2026: cars_commerce acknowledged → health flipped error/consecutive_failures 4 → ok/0. UNVERIFIED: the Dismiss button has not been clicked in the browser.",
          ...
          "Key RPCs: v3_derive_customer, v3_coverage_current, v3_coverage_cached, v3_capture_coverage_snapshot, v3_unattributed_groups, v3_unattributed_sync_status, v3_no_signal_tickets, v3_personal_unlabeled_tickets, v3_channels_usage, v3_accounts_usage, v3_orphan_overrides, v3_orphan_override_suggestions, v3_generate_channel_proposals, v3_channel_proposals_pending, v3_tickets_for_channel, v3_tickets_for_override_key. backfill_v3_customer_keys(force,batch) re-derives in 5k batches (force=true after rule edits).",
          "COVERAGE SERVED FROM CACHE (2 Sep 2026): the Coverage and Unattributed tabs call public.v3_coverage_cached(max_age_minutes), which returns the persisted snapshot and recomputes at most hourly — v3_coverage_current() was the single most expensive statement in the DB (~2.3s mean, ~7.3s max, 249 calls) because it ran live on every render. The live function is untouched and still callable for on-demand truth. Trade-off stated plainly: coverage can lag reality by up to an hour, accepted because attribution moves in days, not minutes. Migration 0034 (4 Sep 2026) then narrowed the SQL itself: v3_coverage_current() selects only the columns its counters need (it was materialising raw_payload) and computes registry membership once; v3_unattributed_groups() precomputes its classification/probe expressions once. Grants preserved, output equivalence checked; coverage measured at ~74ms vs a ~2.28s historical mean. UNVERIFIED: unattributed-group equivalence at scale (only 3 unattributed rows exist today).",
    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "UI /customer-report (CustomerReport.tsx, nav 'Customer report', Beaker/experimental) — PROTOTYPE, out to CSMs for feedback. A per-customer SLA + volume view for CSM-style consumption. AUDIENCE/WHY: CSMs do NOT have Intercom access, so ALL detail is self-contained in-app — Intercom conversation IDs render as plain SELECTABLE TEXT (to quote to support), never as deep-links; the ONE external link is the Escalated Issue (Linear). NUMBERS REUSE THE ENGINE: consumes useSlaBatch + evaluateCompliance (no new SLA computation) so figures match Dashboard/Report exactly; currently-open counts come from a light intercom_tickets_v3 query (lifecycle_status ∈ open / reopened_after_finalize) by customer_key; customer list from v3_customer_accounts excluding is_test. FILTERS: searchable customer combobox; date range (This month default / Last month / 30d / 90d, applied to closed rows via finalized_at/closed_at); 'Show closed issues' toggle (default OFF). SUMMARY CARDS: Currently open (per-severity mini-breakdown), Closed in range (per-severity mini-breakdown; Unclassified rendered ONLY when >0 and styled as an anomaly — closed tickets should always be classified), First Response met % (customer-initiated scope), Resolution met %, Breaches, CSAT positive (% of ratings 4–5 on the 1–5 scale, description carries the response COUNT + average). ESCALATED TO DEV table (independent of the Show-closed toggle — always shows open AND closed escalations): criteria = 'Escalated to Engineering' = Yes OR Ticket type ∈ {Bug, Incident}; columns Subject · Intercom ID · Severity · Type · Esc→Eng · Linked issue · State · Created. Linked issue reads the 'Escalated Issue' custom attribute (legacy 'Linear Issue' fallback) and renders a clickable Linear link showing the issue id (e.g. ENT-2804). DELIBERATE: it does NOT scrape notes for Linear URLs — a URL in a note may be a related investigation, not the filed issue. THEMES — PRODUCT AREAS / TICKET TYPES panels (always, under the summary cards): two distribution cards over the SAME population the Total tickets card counts (open tickets + closed tickets in range), each row label · count · % with a proportional bar, sorted descending, Unclassified pinned LAST and muted so a labelling gap reads as a gap, not a theme. DETERMINISTIC BY DESIGN — no AI call, no credits, no extra query; it aggregates fields already loaded for the tables. ATTRIBUTE SOURCE OF TRUTH (panels + both tables): Intercom custom attributes win — product area = 'Affected Product Area' → 'Product Area' → mirrored product_area column; ticket type = 'Ticket type' → 'Type' → mirrored classification column. Mirrors are a FALLBACK ONLY (the window between a Hub write and the next sync refreshing the attribute blob). Same chain as MyQueue.tsx — keep them in step. OPEN ISSUES table (always): Subject · Intercom ID · Severity · Type · Product area · Created · Last activity (intercom_updated_at) · State — no outcome columns, open tickets have no resolution yet. CLOSED ISSUES table (only when Show-closed is ON): Subject · Intercom ID · Severity · Type · Product area · Created · Resolved · First Response (value + met/breach) · Resolution (value + met/breach); closed rows read attributes from raw_payload already fetched by useSlaBatch — no additional query.",
