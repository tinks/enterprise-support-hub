    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "VERIFICATION (2 Sep 2026): negative — anon-key POST to /rest/v1/rpc/v3_coverage_current returns 401 `permission denied for function`; sync-knowledge-pending returns 401 with no header and 401 with the anon key alone (metadata-IP sourceUrl never reached). Positive — has_function_privilege confirms `authenticated` and `service_role` retained EXECUTE on the same functions, and build is clean. The editor-session-with-non-allowlisted-host 400 branch is UNVERIFIED (no minted editor session in this pass). Batch B (the ~32 cron/webhook mutation functions), the Gmail OAuth `state` binding, and the private attachment bucket remain deliberately open.",
        ],
        accent: "default",
      },
    },
      ...
      data: {
        ...
        details: [
         ...
         "THREE-WAY SPLIT (engine v2, 1 Sep 2026): customer wait is no longer a residual. resolution_customer_wait_s / _bh_s and resolution_window_s are persisted alongside active and closed, all four walked from the SAME timeline in sla-core.ts, so active + closed + customer_wait = window BY CONSTRUCTION — any drift is an engine bug, not a source mismatch. Raw (Intercom time_to_last_close) stays outside the identity as reconciliation only.",
          "VERIFIED after the forced re-backfill of all 592 finalized rows: identity violations = 0; nulls unchanged at 2 (the same not-computable rows); resolution_active_s / _bh_s / _closed_s byte-identical to their pre-backfill snapshot on all 592 rows (0 changed). Population medians: active 2.25h vs customer wait 49.97h — 2,417.7 ticket-days of customer wait against 86.8 ticket-days of closed time, which is why the residual was never a safe proxy. No reporting surface reads the new columns yet; that is a separate pass.",
