    ...
    },
    {
      ...
      data: {
        ...
        details: [
          ...
          "UI /sla-workbench (SlaWorkbench.tsx) — practitioner detail / data-behind-it. Tab 1 'Analyze by ID (live)': paste ≤10 ids → sla-ticket-analyze → per-ticket card with headline strip (our human FRT calendar+BH vs Intercom time_to_admin_reply + Δ + Intercom SLA status), color-coded timeline, calendar|BH metric table, origin+flag badges, prominent internal/no-customer warning. Tab 2 'Batch (stored)': snapshot-based (~287 rows, not live — same rationale as before). ComplianceSection at TOP with per-severity columns N, FR target, FR %met, FR breaches, FR n/a, Res target, Res %met, RES BREACHES (mirrors FR breaches — right-aligned destructive, '—' when 0, Sev 4 '—'). Customer-initiated↔All-tickets basis toggle for FR only. BY-SOURCE breakout (collapsible): rows Slack · Sam-first · Direct (email/messenger, no Sam) + greyed Manually-logged (excluded); columns n · FR %met · Res %met · Pre-inbox median; source per row = origin==='slack' → Slack, else sla.flags.samParticipated → Sam-first, else Direct. Why by-source: the aggregate hid three very different populations — Sam-first strongest, Slack weakest with large pre-inbox lag. Data fact: Sam first-lines EMAIL/MESSENGER only (0 public Sam replies on Slack-sourced tickets); on Slack, humans reply directly. KPI tiles below the table: Human FR bh + calendar (inbox-anchored), first response any-agent, TTR bh, Pre-inbox time (labelled 'process signal, not an SLA'). Full sortable per-ticket table + Corrected/Legacy toggle preserved.",
