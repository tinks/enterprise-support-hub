...
- Table `public.inbox_v2_tickets` keyed by `intercom_conversation_id` (unique). Stores Intercom-sourced `owner` (via `admin_owner_map`), `product_area` (custom_attributes["Affected Product Area"]), `classification` (custom_attributes["Ticket type"]), `tags` (text[] mirroring Intercom conversation tag names), `status`, contact info, timestamps, the full `raw_payload` jsonb for debugging, plus locally-owned engagement columns (`engagement_override`, `engagement_override_at`, `engagement_override_by`, `engagement_ai_guess`, `engagement_ai_reason`, `engagement_ai_at`). RLS: authenticated read AND update (overrides); the sync function (service_role) writes Intercom-sourced columns only — engagement columns are never touched by sync.
