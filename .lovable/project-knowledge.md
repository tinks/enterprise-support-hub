...
**Paused in the UI (9 Sep 2026).** Pax refused the request with "I couldn't verify you as a lovable.dev user" — it only answers verified human `lovable.dev` Slack members, and the Ask Lovable app identity is not one. Rather than reuse a personal identity as a shared bot (blast radius, misleading attribution, offboarding risk), the feature is **paused at the UI layer only**: `PAX_DISABLED = true` in `src/components/issues/PaxInvestigateControl.tsx` disables both the initial request and the Retry-link action and shows the reason inline. The control stays mounted so the concept is demonstrable. Backend, schema, cron-free invocation path and audit logging are untouched and still deployed. **Unblocking requires a dedicated automation Slack member** with a verified `@lovable.dev` email, scoped to `#pax-ets-help` with minimum posting scopes and no admin/private-channel access (working name `esh-investigations`); Pax-side allowlisting of the app is being pursued in parallel. Flip `PAX_DISABLED` to `false` once that account's token replaces `SLACK_BOT_TOKEN` for this function.
