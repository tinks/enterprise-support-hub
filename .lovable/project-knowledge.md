...
- **Contact naming (21 Sep 2026):** `users.info` is now always called (previously only when no email was passed in) and returns both `profile.email` and a display name (`profile.real_name` → `profile.display_name` → `user.real_name`). Email path is unchanged (contact created/updated by email, name = email). When Slack exposes **no** email — typical for Slack Connect guests — the contact is created with `external_id = <slack user id>` and `name = <real Slack name>`, falling back to the old `Slack User U…` placeholder only when Slack returns no name at all. If a no-email contact already exists and its name is empty or the exact `Slack User U…` placeholder, it is PUT-updated to the real name once; any other existing name is left alone. The whole lookup is inside try/catch — a Slack API failure leaves `resolvedSlackName = null` and restores the previous behaviour exactly, so ticket creation never breaks. Scope: one file, `slack-interactions/index.ts`, trivially revertible. Does not change `conversation_mappings` (which already stored `slack_user_name`) or channel attribution.
...
- **Relay-only roster class (`role='csm'`, Aug 2026).** CSMs will never be Intercom admins, so `teammates.intercom_admin_id` is now **nullable** and `role` accepts `'csm'` (constraint: a row must carry at least one of admin id / Slack id / email). A CSM sits on the roster purely so `slack-events` can RESOLVE who spoke: their Slack reply still relays under the fallback relay admin (Sam) with the `[From: … via Slack]` marker, but **no identity gap is recorded** — the gap reason is now `not_on_roster` and fires only when the sender matches no active roster row at all (previously `no_intercom_admin_id`, which fired for every admin-id-less teammate). **FRT impact — intentional:** the First-Response roster is `role='support'` ONLY, so a CSM reply does **not** stop the First-Response clock, and (posting under Sam) reads as `sam_ai` to the engine either way. Same for resolution/cadence "our reply" predicates. Diana (`U0AK8TF9P0D`, `diana@lovable.dev`) is seeded as the first `csm` row and her gap row was marked resolved. **UNVERIFIED:** no live Slack reply from a `csm` row has been relayed since the change.
 ...
 - **Route fix (2026-09-17).** `relay_identity_gaps` pointed at `route: "/settings"` / `routeLabel: "Settings (teammates)"`, but teammate identity (`intercom_admin_id`) is edited on **`/people`** — the admin-only sidebar entry in `AppLayout.tsx`, registered in `App.tsx`. The signal in `src/lib/actionSignals.ts` now routes to **`/people`** with label **`Admin → People`**, so the Action Center button lands on the page that actually fixes the gap. Presentation-only; no signal logic, query or count changed.
...
- **Linear phase 2 (LIVE, 2026-08-24) — `sync-linear-escalations`.** Cron `sync-linear-escalations-daily` at **05:50 UTC**, plus an on-demand **Sync Linear** button on the board (editor-gated). **Read-only against Linear**: it never writes to Linear, and in the Hub it writes **only** `linear_key / linear_title / linear_state / linear_assignee / linear_synced_at` — `hub_state`, `note`, `owner` and `linear_url_override` stay human-owned. Keys are resolved with the **same priority chain as the UI**, split into team key + issue number, and looked up one at a time through the connector gateway (`POST /linear/graphql`, `issues(filter:{team:{key:{eq}}, number:{eq}}})`); capped at **200 keys per run**. It **does** insert rows for referenced tickets that have no Hub row yet (`hub_state` defaults to `open`) — a deliberate narrowing of the virtual-open-rows rule, so live Linear metadata does not require a human touch first. A gateway non-200 **aborts the run** and is surfaced with status + body so a bad token can never read as "issue not found"; unresolvable keys are returned in `not_found`. Health key `linear_escalation_sync`. First run: 32 candidates, 30 distinct keys, **27 resolved, 29 rows written, 3 not found** (`AGE-993`, `INTX-1634`, `SCA-2893` — most likely teams the connection's token cannot read; **UNVERIFIED** which). The board renders the mirror only when `linear_key` matches the currently resolved key, so an edited link never shows stale Linear metadata.
 - **Moved Linear issues (2026-09-14) — identifier-first lookup.** `sync-linear-escalations` now resolves each key with `issue(id:"EVERY-2655")` **first**; Linear follows an issue that Dev moved to another team, which the old `issues(filter:{team,number})` query could never do — that query is kept only as a fallback. The mirror is written under the **current** identifier (`linear_key = issue.identifier`) on both `dev_escalations` and `dev_escalation_links`, and the run response returns `moved[]` (old → new). **VERIFIED 2026-09-14 live:** 62 candidates / 55 distinct keys / **55 resolved, 0 not found** (previously 5 not found); moved = `SCA-2893→ENT-3100`, `INTX-1634→SEC-843`, `EVERY-2655→FOU-2`, `CLO-1259→SCA-3592`, `AGE-993→EVERY-2499`. Intercom `215475543627751` now carries `FOU-2` (Backlog) where it had no `dev_escalations` row at all. Old link rows keyed by a retired identifier are left in place; **UNVERIFIED** whether any stale duplicate affects the eng-wait union.
 - **Hourly Linear refresh (2026-09-17) — staleness fix.** The once-daily `sync-linear-escalations-daily` job was **unscheduled** and replaced by an **hourly** cron at **`50 * * * *`** (job id 51, first fire 19:50 UTC 2026-09-17), authenticating with the protected cron secret via `public.esh_cron_headers()` rather than the anon key. Cause: ticket `215475443059110` (Tejas, `AI-800` → canonical `CA-2397`) showed **Triage** in My Queue while Linear already read **In Review** — My Queue renders the Hub mirror, never Linear directly, so a 24h cadence let the mirror age up to a full day behind. Cadence chosen over 15-minute (needless gateway load; Linear transitions are human-paced) and 4-hourly (still visibly stale within a support shift). An editor-gated **Sync Linear** button on `MyQueue.tsx` runs the same function on demand with loading/toast/error handling and a queue reload afterward. Changelog `c2da8ac3-f819-49d4-a2bb-e54a10610d29`. **UNVERIFIED:** cron internals are not readable from the app; the first unattended fire at 19:50 UTC should be confirmed by a fresh `linear_synced_at` with nobody clicking the button.
...
### Signals only load when signed in (14 Sep 2026)

`ActionSignalsProvider` wraps the whole router in `src/App.tsx`, including `/login`, so on a signed-out page load every signal loader fired as the `anon` role. Two of them call `SECURITY DEFINER` RPCs granted to `authenticated` only (`v3_unattributed_groups()`, `v3_channel_proposals_pending()`), producing recurring `permission denied for function ...` entries in the Postgres logs. The provider now subscribes to `supabase.auth.onAuthStateChange` plus `getSession()` and refuses to run `load()` until a session exists; the focus/visibility refetch path inherits the same guard. Migration `0046_grant_execute_action_signal_rpcs` makes the intended grant explicit (`GRANT EXECUTE ... TO authenticated`) and explicitly `REVOKE`s `anon` on both functions, so a future `CREATE OR REPLACE` cannot silently widen or drop access. No signal logic, count or mute behaviour changed for signed-in users. Verified 14 Sep 2026: typecheck and build clean; signed-in Action Center unchanged.
...
**Controls** (detail sheet, editor-gated by `useCanEdit`): *Mark followed up* applies the lifecycle default; manual overrides are **+1d / +3d / +1w / custom date**; *Acknowledge resolution* / *Undo* clears a resolved escalation out of the bucket. Every stamp records the signed-in email. **Nothing is written to Intercom, Linear or any reporting column**, and Dev-resolved exit is deliberately manual — no auto-clearing from Intercom activity, because the cross-team comms process is not yet defined.
...
### Latest internal note card + Hub ticket snooze (11 Sep 2026)

Two changes driven by ticket 215475781288266, where the panel showed the customer's 4 Sep reply but not Matt's 8 Sep note ("No action needed until Sept 21st…") and the queue therefore insisted it was *Action needed*.

**Latest internal note card.** `ticketComments()` now also returns `note`: the newest `conversation_parts` entry with `part_type === "note"` and non-empty stripped body. `TicketCommentCard` gained `tone="note"` (amber border/background) and `TicketDetailContent` renders it below *Latest reply* as **Latest internal note**. Automated bookkeeping notes are suppressed by `isAutomatedNote()` — author matching `/lovable support/i`, or a body containing `slack.com/archives/` (the Pax permalink note the Hub posts itself). Customer-facing extraction is unchanged: notes still never appear as *Latest reply*, and no note advances `last_admin_reply_at` or any SLA clock.

**Hub-only snooze.** Migration **0045** (`v3_ticket_snooze`) adds four nullable columns to `intercom_tickets_v3` — `snoozed_until`, `snoozed_at`, `snoozed_by`, `snooze_reason` — plus a partial index on `snoozed_until`. Hub-owned: never synced from Intercom, never written back, never read by `sla-core` or any reporting metric.

Semantics in `MyQueue.tsx`: a row is snoozed when `snoozed_until > now()`. It **keeps its natural bucket** (an *Action needed* ticket stays *Action needed*) but sorts below every unsnoozed row, renders at `opacity-50`, and shows *Snoozed until d MMM* in the state cell. A ninth top-level **Snoozed** stat card counts and filters them. When the date passes the row simply stops being snoozed on the next load — no cron, no job.
...
**1. `sla-ticket-analyze` caller scope.** The function proxies arbitrary Intercom conversation IDs through the server's Intercom token for `/sla-workbench` ▸ *Analyze by ID (live)*. It gated on `requireUser`, so any signed-in account — including read-only ones — could pull any conversation. Now `requireEditor` (`public.can_edit`). The ID list is deliberately **not** constrained to rows already in `intercom_tickets_v3`: pasting a just-opened ticket, one outside the synced inboxes, or one below the historical sync floor is the normal use of the tool, and a membership check would silently break all three.

**2. `sam_ticket_reviews` read scope (migration 0050).** The SELECT policy `Authenticated can read sam reviews` used `USING (true)` while insert/update/delete already required `public.can_edit(auth.uid())`. Dropped and replaced with `Editors can read sam reviews` on the same predicate, so review notes are readable only by the practitioners who can write them. Verified in `pg_policies`: all four policies now carry `can_edit(auth.uid())`.
...
The scanner re-flagged two of the three items because it does per-function static analysis and does not resolve shared helpers or infer caller scope. Both are now closed in-file.

**1. `sla-ticket-analyze` scope guard.** The function now resolves the normalized ID list against the Hub first: a service-role client queries `intercom_tickets_v3.intercom_conversation_id` and `conversation_mappings.intercom_conversation_id` for the requested IDs. Only known IDs are fetched from Intercom; unknown IDs return a per-ID `404` result (`"Ticket not found in synced ESH records — it may not have synced yet (open sync runs every 5 minutes)"`) instead of being proxied. `requireEditor` still applies. Accepted tradeoff: a ticket created in Intercom within the last ~5 minutes cannot be analyzed until `sync-v3-open` (or a manual "Sync open") picks it up — SLA analysis is inherently retroactive, so this window is not a practical loss. A failed scope lookup returns `500 scope_check_failed` rather than falling open.
...
**Posting.** `ask-pax-investigate` accepts optional `humanInfoBlock` (1–3500 chars, must start `*Human Info`), prepended above the unchanged Pax request. Pax template, idempotency and the Intercom internal note are unchanged; the audit payload records `human_info`. **Two actions (fix 23 Sep 2026).** The Investigation card shows "Ask Pax to investigate" (primary, one-click original direct start, no extraction) and a separate "Draft Dev Escalation" (outline) that runs extraction → review → post. **Insufficient data.** `summary.insufficient_data` (strict schema, server-enforced): when the thread lacks a concrete component AND failure, the summary is returned blank and the panel shows "Not enough detail in the thread to draft a summary" rather than filler; the required-summary gate forces a human entry.
