...
- CSV export (`ExportPopover` in `InboxV2.tsx`): "Export CSV" button opens a popover with date-range presets (Last 7/14/30 days, This month, Last month, Custom) and From/To date inputs. Queries `inbox_v2_tickets` directly (not capped by the 500-row table fetch), filtered by `intercom_created_at` between the selected dates, paginated in 1000-row batches via `.range()`. Current screen filters (Owner / Product Area / Classification / Status) are applied server-side; Tags and free-text search are applied client-side after the fetch (tags is `text[]`, search spans multiple cols). Columns: Intercom ID, Subject, Contact name/email, Owner, Product area, Classification, Tags (semicolon-joined), Status, Created, Updated. Filename `inbox-v2-YYYY-MM-DD_to_YYYY-MM-DD.csv`. Created date is exported even though it's not a visible column in the table.
