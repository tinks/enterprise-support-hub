...
| Linker priority order (auto-import) | `intercom-webhook` | 1) email→Gmail thread, 2) subject→Gmail thread (24h), 3) subject→`manual_conversations` (7d) — exactly-one match logs `subject_match_existing_manual` and returns without creating a new manual row, 4) create new manual row |
| Thread-overwrite guards | `intercom-webhook` | Both Gmail linkers (email + subject) check siblings on the target `gmail_thread_id`. If any sibling already has a non-null `intercom_conversation_id` that differs from the incoming ticket, log `email_link_conflict` / `subject_link_conflict` and return without overwriting. The thread-level UPDATE also uses `.or("intercom_conversation_id.is.null,intercom_conversation_id.eq.<newId>")` as defense-in-depth. The new Intercom ticket is treated as a duplicate; our DB already represents the conversation correctly |
