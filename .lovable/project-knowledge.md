...
- Table `public.inbox_v2_tickets` keyed by `intercom_conversation_id` (unique). Stores Intercom-sourced `owner` (via `admin_owner_map`), `product_area` (custom_attributes["Affected Product Area"]), `classification` (custom_attributes["Ticket type"]), `tags` (text[] mirroring Intercom conversation tag names), `status`, contact info, timestamps, and the full `raw_payload` jsonb for debugging. RLS: authenticated read-only; only the sync function (service_role) writes.
- Edge function `sync-inbox-v2` searches the enterprise inbox for conversations updated within `windowHours` (default 24), fetches each full conversation, and upserts. Tags come from `icData.tags.tags[].name`. Unlike the live tables it intentionally nulls-out / overwrites values when Intercom is blank — drift is the signal we want.
...
- UI `src/pages/InboxV2.tsx`: read-only table with search + filters (Owner / Product Area / Classification / Status / Tags — first three include a "missing" option, Tags includes a `(no tags)` option), a configurable "Window (h)" input (1–8760) feeding a "Sync now" button, plus a "Backfill 30d" shortcut that calls `sync-inbox-v2` with `windowHours=720`, and a right-side drawer with a "View in Intercom" link. Tags render as badges in their own column and in the detail drawer. Columns are user-resizable via a drag handle on the right edge of each header; widths persist to `localStorage` under `inbox-v2-col-widths` and a "Reset columns" button restores defaults.
- CSV export (`ExportPopover` in `InboxV2.tsx`): "Export CSV" button opens a popover with date-range presets (Last 7/14/30 days, This month, Last month, Custom) and From/To date inputs. Queries `inbox_v2_tickets` directly (not capped by the 500-row table fetch), filtered by `intercom_created_at` between the selected dates, paginated in 1000-row batches via `.range()`. Current screen filters (Owner / Product Area / Classification / Status) are applied server-side; Tags, free-text search, and Engagement are applied client-side after the fetch. Columns: Intercom ID, Subject, Contact name/email, Owner, Product area, Classification, Tags (semicolon-joined), Status, Engagement, Created, Updated. Filename `inbox-v2-YYYY-MM-DD_to_YYYY-MM-DD.csv`. Created date is exported even though it's not a visible column in the table.
- Engagement classification (Inbox v2 only): a ticket is "No engagement" if its `tags` include `enterprise-fyi` or `enterprise-duplicate` (case-insensitive). Pure client-side derivation via `isNoEngagement()` — no schema change. Surfaced as an "Engagement" table column (badge), an "Engagement: any / Engaged / No engagement" filter dropdown, and an "Engagement" CSV column. Trigger tags live in the `NO_ENGAGEMENT_TAGS` set at the top of `src/pages/InboxV2.tsx`.
