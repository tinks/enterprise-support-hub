...
- Table: `public.inbox_v2_tickets`, one row per Intercom conversation, keyed by `intercom_conversation_id`. Authenticated users can read; only `service_role` writes.
- Sync: edge function `sync-inbox-v2`. Pulls conversations in the enterprise inbox updated within `windowHours` (default 24h). Owner from `admin_owner_map[admin_assignee_id]`; product_area from `custom_attributes["Affected Product Area"]`; classification from `custom_attributes["Ticket type"]`. Unlike the live tables, this table DOES null-out values when Intercom is blank — drift is the signal we want to see.
