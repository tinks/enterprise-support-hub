...
description: Subject-based fallback in intercom-webhook for emails relayed via the enterprise-support Google Group, plus 5-min reply reconciliation cron
type: feature
---
When inbound mail arrives via the `enterprise-support@lovable.dev` Google Group, the customer's email is stripped before being relayed to our inbox. The Gmail row's `from_email` is the group alias and Matt/etc. never appear on the row. Email-based linking in `intercom-webhook` therefore cannot find the matching Gmail thread and falls through to creating a duplicate `manual_conversations` entry.

The webhook now applies a subject-based fallback after the email lookup fails: it normalizes the Intercom ticket subject (strips `Re:`/`Fwd:`/`Fw:`, collapses whitespace, lowercases) and matches it against unlinked `gmail_conversations` received in the last 24 h. If exactly one distinct `gmail_thread_id` matches, the ticket is linked to the entire thread. If 0 or >1 match, it logs `subject_link_ambiguous` and falls through.
